...
channels: 먼저 [내 채널]의 콘텐츠 '유형'을 한 단어로 규정하세요(예: 친구 먹방 브이로그 / 일상 브이로그 / 정보·리액션 클립 / 강의). 그런 다음 후보 중 '그 유형과 같은' 채널만 3~6개 고르세요(name은 후보에 있는 그대로).
핵심 규칙:
 · 유형(형식)이 1순위입니다. 주제·소재가 같아도 유형이 다르면(예: 내 채널은 먹방 브이로그인데 후보는 정보·리액션 클립) 조회수가 아무리 높아도 반드시 제외.
 · '[같은형식 추정]' 표시가 붙은 후보를 우선 검토하되, 소개·대표영상 제목으로 실제 유형을 직접 확인하세요.
 · 화자/구성(혼자 말하기 vs 친구끼리 vs 내레이션 클립)도 비슷해야 합니다.
 · 방송사·뉴스·공식·거대 클립 채널 제외. 같은 유형 peer가 없으면 빈 배열([]).
...
export default async (req: Request) => {
  ...
  try {
    ...
    const QSTOP = new Set([
      "콘텐츠", "채널", "유튜브", "영상", "구독", "좋아요", "그리고", "및", "관련", "전문", "형",
    ]);
    const nicheKw = (niche || "")
      .split(/[\s·,/]+/)
      .map((w) => w.replace(/형$/, "").trim())
    ...
    // 형식/결 일치 — 분야 키워드(먹방·브이로그·일상 등)가 채널명·소개·대표영상에 드러나면 같은 유형으로 본다.
    const fmtSet = nicheKw.map((w) => w.toLowerCase()).filter((w) => w.length >= 2);
