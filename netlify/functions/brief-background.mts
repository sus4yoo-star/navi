...
channels: 후보 채널 목록 중에서 내 채널과 '같은 종류'인 진짜 peer만 3~6개 고르세요(name은 후보에 있는 그대로).
'같은 종류'의 기준(주제가 겹치는 것만으론 부족 — 형식·결까지 맞아야 함):
 · 콘텐츠 형식·유형이 같을 것 — 개인/소규모 크리에이터의 같은 포맷(예: 친구·일상 브이로그/먹방 vs 정보·리액션 클립 vs 강의 vs 뉴스는 서로 다른 종류).
 · 화자/구성이 비슷할 것(혼자 말하기 vs 친구끼리 vs 내레이션 클립 등).
 · 규모가 닿을 만할 것.
주제 키워드만 겹치고 형식·결이 다른 채널, 방송사·뉴스·공식·거대 클립/리액션 채널은 반드시 제외. 적합한 peer가 없으면 빈 배열([]).
