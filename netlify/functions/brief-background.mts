...
const SYSTEM = `당신은 '나비', 한국 유튜브 크리에이터에게 '무한한 영감'을 주는 AI 성장 PD입니다. 크리에이터가 '우물 안 개구리'가 되지 않도록,
바깥의 '영감을 주는 채널들'에서 차용할 기법을 길어오고 → 내 위치를 진단하고 → 이번 주 만들 영상과 전략을 한 줄기로 제시합니다.
가장 중요한 것: 단순 분석에 그치지 말고, 크리에이터의 가슴에 꽂히는 '영감'을 줄 것. 그가 이미 쥐고 있는 무기(잘 된 데이터)와 바깥에서 길어온 기법을 연결해, '아, 이거다' 싶은 확신과 다음 한 걸음을 주세요.
근거는 오직 [내 채널], [후보 채널들], [그 채널들의 시청자 댓글]뿐입니다. 수치·사실·댓글을 지어내지 마세요. 단, 톤은 분석가가 아니라 곁에서 밀어주는 PD처럼 — 따뜻하고 확신 있게. 빈말·과장·이모지·클리셰는 금지(영감은 구체에서 나온다).
ideas는 영감 채널에서 지금 먹히는 기법과 댓글에 드러난 시청자 수요에 근거하되, 내 채널의 색깔·톤으로 변주하세요. 모든 출력은 한국어.
아래 JSON 하나만 출력. 그 외 텍스트·코드펜스 금지.
{
 "spark":"오늘 이 크리에이터의 가슴에 꽂을 영감 한 방 — 짧고 강한 1~2문장. 그가 이미 가진 강점(잘 된 영상의 '패턴/매력')을 일깨우고 다음 한 걸음으로 밀어주는 말. 수치 나열·영상 제목의 모호한 축약(예: '오이 영상')은 금지. 영상을 가리킬 땐 제목의 핵심어를 그대로 따옴표로, 무엇이 통했는지가 분명하게.",
...
channels: 내 채널이 '차용할 수 있는 영감'을 주는 채널 4~6개를 고르세요(name은 후보에 있는 그대로). 비즈니스에서 타 분야 벤치마킹으로 성공하듯, 꼭 같은 분야가 아니어도 됩니다.
 · 같은/인접 분야 채널을 우선하되, '다른 분야'여도 훅·제목 패키징·시리즈 구성·편집 리듬·커뮤니티 운영 등 내 채널에 옮길 수 있는 '뚜렷한 기법'이 있으면 포함.
...
export default async (req: Request) => {
  ...
  try {
    ...
    // 분야(niche)에서 '결/형식' 키워드 추출 — 먹방·브이로그·일상 등(주제가 아니라 '어떤 채널인지')
    const QSTOP = new Set([
      "콘텐츠", "채널", "유튜브", "영상", "구독", "좋아요", "그리고", "및", "관련", "전문", "형", "친구와의", "친구",
    ]);
    const FORMAT_VOCAB = [
      "먹방", "브이로그", "vlog", "일상", "리뷰", "언박싱", "챌린지", "요리", "쿡", "운동", "헬스",
      "게임", "메이크업", "뷰티", "공부", "스터디", "여행", "캠핑", "육아", "수다", "토크", "쇼핑", "하울", "꿀팁", "정보",
    ];
    const nicheWords = (niche || "")
      .split(/[\s·,/]+/)
      .map((w) => w.replace(/형$/, "").trim().toLowerCase())
      .filter((w) => w.length >= 2 && !QSTOP.has(w) && !/^\d/.test(w));
    // 형식 단어(먹방·브이로그 등)를 앞으로 — 검색·형식매칭에 우선 사용
    const fmtWords = nicheWords.filter((w) => FORMAT_VOCAB.some((f) => w.includes(f) || f.includes(w)));
    const nicheKw = [...new Set([...fmtWords, ...nicheWords])].slice(0, 4);
    // 주제(해시태그)와 결(분야 키워드)을 '번갈아' 넣어 둘 다 발굴되게 한다.
    const maxLen = Math.max(topTags.length, nicheKw.length);
    for (let i = 0; i < maxLen; i++) {
      if (topTags[i]) queries.push(topTags[i]);
      if (nicheKw[i]) queries.push(nicheKw[i]);
    }
    if (niche && norm(niche) !== norm(myName)) queries.push(niche);
    for (const v of topByViews.slice(0, 2)) {
    ...
    const uniqQ = queries
      ...
      .slice(0, 6); // search는 호출당 100유닛 — 주제+결 다양성 위해 최대 6개
    ...
    // 형식/결 일치 — 형식 단어(먹방·브이로그·일상 등)가 채널명·소개·대표영상에 드러나면 같은 유형으로 본다.
    const fmtSet = (fmtWords.length ? fmtWords : nicheKw).filter((w) => w.length >= 2);
    const fmtMatch = (c: any) => {
      if (!fmtSet.length) return 0;
      const t = `${c.name} ${c.description || ""} ${c.top?.title || ""}`.toLowerCase();
      return fmtSet.some((w) => t.includes(w)) ? 1 : 0;
    };
    const scored = [...enriched]
      .filter((c) => c.subs <= subCap)
      .map((c) => ({ ...c, kw: kwOverlap(c), fmt: fmtMatch(c) }));
    const cmp = (a: any, b: any) => {
      const act = (b.recent60 > 0 ? 1 : 0) - (a.recent60 > 0 ? 1 : 0);
      if (act) return act;
      if (b.fmt !== a.fmt) return b.fmt - a.fmt; // 같은 형식(먹방·브이로그 등) 우선
    ...
    // 후보 풀 = 관련/형식 우선 10 + 잘 나가는 채널(타 분야 포함) — 차용할 '영감'까지 섞는다.
    const byRel = [...scored].sort(cmp);
    const byPerf = [...scored].sort((a, b) => b.avgViews - a.avgViews);
    const candidates: any[] = [];
    const picked = new Set<string>();
    const add = (c: any) => {
      if (c && !picked.has(c.id) && candidates.length < 14) {
        picked.add(c.id);
        candidates.push(c);
      }
    };
    byRel.slice(0, 10).forEach(add);
    byPerf.forEach(add);
    ...
    const candText = candCards
      .map(
        (c, i) =>
          `${i + 1}. ${c.name}${c.fmt ? " [같은형식 추정]" : ""} · [${catLabel(c.category)}] · 구독 ${c.subs.toLocaleString()} · 최근60일 ${c.recent60}편 · 평균조회 ${c.avgViews.toLocaleString()} · 쇼츠 ${c.shortsPct}%` +
