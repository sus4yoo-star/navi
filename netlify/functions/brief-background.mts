...
channels: 먼저 [내 채널]의 콘텐츠 '패밀리'를 규정하세요(예: 먹방/일상 브이로그). 그런 다음 후보 중 '같은 패밀리'의 채널을 4~6개 고르세요(name은 후보에 있는 그대로). 너무 좁히지 말 것 — 세부 형식(혼자 vs 친구)이 조금 달라도 같은 패밀리면 포함.
규칙:
 · 같은 콘텐츠 패밀리(예: 먹방·일상·브이로그 계열)면 포함. '[같은형식 추정]' 표시와 소개·대표영상으로 판단.
 · 명백히 다른 유형(뉴스·강의·정보/리액션 클립·방송사·공식·거대 클립)만 제외. 주제만 같고 패밀리가 전혀 다르면 조회수 높아도 제외.
 · 가능하면 4개 이상 보여주되, 같은 패밀리가 정말 적으면 있는 만큼만(최소 기준은 '패밀리 일치').
